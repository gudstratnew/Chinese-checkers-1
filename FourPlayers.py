...
def FoorPlayers(p1, p2, p3, p4):
    ...
    first_player = [[4, 18], [4, 20], [4, 22], [4, 24], [5, 19], [5, 21], [5, 23], [6, 20], [6, 22], [7, 21]]
    ...
    first_aim = [[12, 0], [11, 1], [12, 2], [10, 2], [11, 3], [12, 4], [9, 3], [10, 4], [11, 5], [12, 6]]
    second_aim = [[4, 0], [4, 2], [5, 1], [4, 4], [5, 3], [6, 2], [4, 6], [5, 5], [6, 4], [7, 3]]
    third_aim = [[4, 24], [5, 23], [4, 22], [6, 22], [5, 21], [4, 20], [7, 21], [6, 20], [5, 19], [4, 18]]
    fourth_aim = [[12, 24], [12, 22], [11, 23], [12, 20], [11, 21], [10, 22], [12, 18], [11, 19], [10, 20], [9, 21]]
    ...
    class remplissage:
        ...
        def pion(self):
            colors = [(240, 230, 230), "red", "yellow", "orange", "green"]
            for i in range(0, 17):
                for j in range(0, 25):
                    if matrix[i][j] >= 0:
    ...
    def winner():
        ...
        fourth = True
        for i in range(len(first_player)):
            if matrix[first_player[i][0]][first_player[i][1]] != 3:
                third = False
                break
        for i in range(len(second_player)):
            if matrix[second_player[i][0]][second_player[i][1]] != 4:
                fourth = False
                break
        for i in range(len(third_player)):
            if matrix[third_player[i][0]][third_player[i][1]] != 1:
                first = False
                break
        for i in range(len(forth_player)):
            if matrix[forth_player[i][0]][forth_player[i][1]] != 2:
                second = False
                break
    ...
    def heuristic(state, pid):
        ...
        player1_inverse = [[12, 0], [12, 2], [11, 1], [12, 4], [11, 3], [10, 2], [12, 6], [11, 5], [10, 4], [9, 3]]
        player2_inverse = [[4, 0], [5, 1], [4, 2], [6, 2], [5, 3], [4, 4], [7, 3], [6, 4], [5, 5], [4, 6]]
        player3_inverse = [[4, 24], [4, 22], [5, 23], [4, 20], [5, 21], [6, 22], [4, 18], [5, 19], [6, 20], [7, 21]]
        player4_inverse = [[12, 24], [11, 23], [12, 22], [10, 22], [11, 21], [12, 20], [9, 21], [10, 20], [11, 19], [12, 18]]
